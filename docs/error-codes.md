...
돌리는 것이다. **`BACKBONE_UNAVAILABLE`과도 다르다** — 사진 수업에는 대안이 없어서 그쪽
문구가 *"다시 시도하세요"*인데, 여기는 같은 알고리즘이 순수 JS로 돈다.
...
때문이다 — 그쪽은 프로젝트 파일이고 이쪽은 방금 만든 사진 압축 파일이다. `IMAGE_ZIP_NO_IMAGES`는
...
**숫자로 못 읽는 값도 같은 이유로 시끄럽게 거절한다** — `PREDICTION_INPUT_NOT_NUMBER`
(2026-09-19 R33 C-3). 안 막으면 전처리가 `?? 0`으로 떨어뜨려 **`없음`도 `1,650`도 0이
되고**, 표준화까지 지나면 그 0은 평균에서 한참 떨어진 값이라 **모델이 자신 있게 틀린 답을
낸다.** 실패도 경고도 없다. **닿는 길은 파일 예측이다** — 손으로 치는 칸은 `type="number"`라
브라우저가 막고, 파일 쪽은 열 이름만 보고 받는다. 교실에서 흔한 모양은 글자가 아니라
**천 단위 쉼표**다.

**빈 칸과는 끝까지 나눈다** — 학생이 할 일이 다르다(채우는 것과 고쳐 적는 것).
**쉼표를 받아 주지 않는 이유**는 학습 때 그 열을 `detectKind`가 범주형으로 판정하기
때문이다. 예측에서만 숫자로 읽으면 같은 글자를 학습과 예측이 다르게 해석한다.
...
훈련 데이터의 타깃이 **한 종류뿐이라 갈라 볼 것이 없다.** 값이 한 종류인 열을 타깃으로
놓고 분류를 돌리면 아무도 안 막고, **학습은 실제로 돈다** — 재 보니 정확도 100% ·
F1 100% · 혼동 행렬 1×1이었다.

**거절하지 않는 것이 결정이다.** 전처리 화면이 이미 *"타깃에 값이 한 종류뿐이라 예측할
것이 없습니다"*로 주의를 주고, *"정확도 100%인데 왜 쓸모없을까"*는 그 자체로 좋은 수업
장면이다. **그래도 말은 해야 한다** — 교실에서 100%는 실패가 아니라 성공으로 읽히고,
유일한 단서인 특이도 0%를 학생이 읽어내지 못한다.

**사진의 `IMAGE_TOO_FEW_CATEGORIES`와 판정은 같고 처방이 갈린다.** 그쪽은 거절이다 —
사진에는 이 주의를 미리 띄울 열 목록이 없다.

**엔진이 아니라 실험 층이 붙인다.** 모델의 성질이 아니라 데이터의 성질이라, 엔진마다
넣으면 알고리즘 수만큼 같은 판정이 생기고 새 엔진이 그것을 빠뜨린다. **훈련 몫을 세고
열 전체를 세지 않는다** — 열이 한 종류면 훈련 몫도 반드시 한 종류라 전처리 화면의 주의를
전부 덮고, **분할이 만든 한 종류짜리 훈련 몫**까지 잡는다.

**회귀는 옆의 코드가 받는다.** 군집은 타깃이 비어 있어 아무것도 안 붙는다.
...
**판정은 위와 한 함수이고 코드만 갈린다** (`noVarianceWarning`). 학생이 할 일이 다르기
때문이다 — 분류는 *"갈라 볼 것이 없다"*이고 회귀는 **"오차로 판단하라"**다. RMSE·MAE는
여전히 뜻이 있어서 완벽히 맞힌 선형회귀(0.000)와 크게 틀린 인공신경망(6.484)이 그 값으로는
갈리는데, **결정계수만 둘 다 만점/영점으로 뭉갠다.**

**세는 축이 갈린다.** 분류 타깃은 문자열이라 `Set`으로 세지만, **회귀 타깃은 엔진이
`Number()`로 읽는다** — `42`·`42.0`이 든 열은 문자열로 두 종류인데 수치로는 한 종류이고,
그 열에서 R²의 분모는 정확히 0인데 문자열로 세면 경고가 안 붙는다. 그래서 회귀는 수치로
센다(분산 0과 같다).

> **문구가 *"1.000이 나와도"*라고 말하면 안 된다.** 같은 데이터에서 인공신경망은 0.000을
> 받으므로 모델에 따라 그 문장이 거짓이 된다. 첫 초안이 그렇게 적혀 있었고 R25가 재서
> 잡았다.

> **회귀 쪽은 재고 나서 지표를 고쳤다** (2026-09-03). 상수 타깃으로 회귀를 돌리면 참해가
> `계수 0 · 절편 그 값`이라 R²가 1.0이어야 하고 sklearn이 그렇게 낸다. 그런데 우리는
> **0.000**을 내고 있었다 — `ml-regression-multivariate-linear`이 정규방정식을 써서 계수에
> 1e-14 먼지를 남기고, `metrics.ts`의 R²가 분모 0일 때 **잔차가 정확히 0인지**로 1과 0을
> 가르기 때문이다(그 규칙 자체는 sklearn과 같다). sklearn처럼 **타깃을 센터링해서** 그
> 자리를 맞췄다 — 정상 데이터의 못 박은 값은 하나도 안 움직였다.

**`Run.warning`이 하나뿐이라 겹치면 이것이 이긴다.** 타깃이 한 종류면 그 점수 자체에
뜻이 없으므로 *"덜 다듬어진 계수에서 나온 숫자다"*보다 먼저 할 말이다.
...
**마지막 하나만 할 일이 다르다** (2026-09-03). 인공신경망 **회귀**에서는 전처리
스케일링이 답이 아니라 **해롭다** — 같은 데이터에서 그것을 켜니 R²가 −0.20에서
**−10.2**가 됐다(분류는 같은 조치가 0.40 → 1.00이다). 특성을 표준화하면 은닉층의 활성이
작아져 타깃 크기에 닿는 데 더 오래 걸리기 때문이고, **그래서 코드를 나눴다.** 회귀 문구는
손실 곡선과 타깃의 크기를 가리킨다. `tests/locales.spec.ts`가 **어느 코드에서 스케일링이
답인지를 손으로 적은 목록**으로 지킨다 — 이름 모양(`_NOT_CONVERGED`)으로 세면 처방이
다른 새 코드가 조용히 같은 규칙을 뒤집어쓴다.

**학생이 할 일은 나머지 넷이 같다 — 먼저 전처리 스케일링이다** (2026-08-31). 로지스틱 문구만
`maxIter`를 늘리라고 말하고 있었고, 실물 `.mlpx`에서 그 길이 막다른 길이었다
(`open-decisions.md` "로지스틱 회귀 솔버를 sklearn과 같은 구조로 바꾼다"의 "실물이 꺼낸
것"). 문장은 로케일이 갖고 `tests/locales.spec.ts`가 그 넷을 함께 지킨다.
