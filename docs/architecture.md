...
아니라 V1이 성립하는 조건이다. 엔진 선택(`open-decisions.md` #3)이 **블로킹 결정**인 이유다.
...
| 후보 | 장점 | 단점 |
|---|---|---|
| Pyodide + scikit-learn | sklearn 그대로. 교육적으로도 레퍼런스와 일치 | 최초 10~25MB. 수업 시작에 30명이 동시에 받는다 |
| ml.js 계열 (순수 JS) | 수백 KB. 즉시 시작 | sklearn과 숫자가 다르다. Orange3와도 다르다 |

**"동등성 보장"이라고 말하지 마라.** 같은 sklearn이라도 WASM과 네이티브는 BLAS와 스레드 수가
달라 부동소수점이 미세하게 갈린다. 트리 계열은 `random_state` 고정 시 대개 같지만
SVM·LogisticRegression은 마지막 자리가 다를 수 있다. UI는 자릿수를 제한해 표시하고,
대조는 정확 일치가 아니라 **허용 오차**로 판정한다(`open-decisions.md` #12).
...
엔진이 Pyodide라면 최초 로드가 10~25 MB다. **이걸 GitHub Pages가 서빙한다** — 우리가 부담하는
전송량은 0이고, Pages는 CDN이 붙어 있어 학교 회선에서도 빠르다. 개인 서버로 이만한 정적
자산을 서빙하지 않아도 되는 것이 정적 배포의 부수 효과다.
...
- 남는 위험은 **수업 시작 시각에 30명이 동시에 최초 다운로드**하는 경우다.
  이것이 엔진 선택(#3)에서 가장 무거운 고려사항이다.
