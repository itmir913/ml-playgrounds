...
없어도 열리고, `model/` 아래가 없는 것은 지표만 남은 정상적인 파일이다.
...
> `crypto.subtle`은 **보안 컨텍스트에서만** 존재한다. Pages(https)와 localhost는 되지만
> 자가호스팅을 `http://192.168.x.x:8000`으로 접속하면 없다. 해시는 보안이 아니라 방지턱이므로
> 없으면 생략하고 그렇게 표시한다. 저장이 실패해서는 안 된다.
...
어휘의 단일 출처는 `frontend/src/errors.ts`의 `FILE_HASH_STATUSES` / `REPRODUCTION_STATUSES`다.
