...
클라이언트는 이를 `.mlpx`에 넣는다. 예측 경로는 두 가지다.

| 상황 | 모델이 어디 있는가 | 예측 방식 |
|---|---|---|
| 세션 중 (방금 학습함) | 서버 메모리에 살아 있음 | 요청만 보내면 즉시 응답 |
| 세션 밖 (저장된 파일을 다시 엶) | `.mlpx`의 `model/` | 모델을 함께 보내 새 세션을 연다 |
...
**왜 필요한가:** `.mlpx`는 zip이고 수행평가 제출물이다. 학생이 압축을 풀어 `results.json`의
...
- 교사 화면이 `results.json`에 적힌 `datasetHash`를 그대로 믿으면 검증은 무의미하다.
  **반드시 `.mlpx` 안의 `dataset/`에서 해시를 직접 재계산해 비교해야 한다.**
