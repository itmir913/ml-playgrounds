...
고칠 수 있다. "재학습 없이 결과 확인"이 기본 동작이므로 위조 탐지가 필요하다.
...
- 지표를 손으로 고치는 것 (metrics가 서명 대상이므로 불일치가 드러난다)
...
- 교사 화면이 파일에 적힌 `datasetHash`를 그대로 믿으면 검증은 무의미하다.
  **반드시 `.mlpx` 안의 `dataset/`에서 해시를 직접 재계산해 비교해야 한다.**
- 데이터 원본이 빠진 프로젝트 파일(이미지·음성 후보안)은 위 재계산이 불가능하다.

이것이 **데이터셋 바이트를 절대 가공하지 않는 이유**다. 읽고 쓰면서 인코딩을 바꾸거나
줄바꿈을 정규화하면 해시가 달라져 검증 전체가 무너진다.
