...
타깃 컬럼의 자료형은 학생이 무엇을 하려는지 알려주지 않는다. 학생이 명시적으로 고른다.

`if dataType === 'image'` 같은 분기를 만들지 않는다. 등록부에 항목을 추가하면 화면이 따라온다.

**비활성화하되 숨기지 않는다.** "군집화 모델도 있구나"를 보는 편이 교육적으로 낫다.
꺼진 것에는 **왜 못 쓰는지 이유가 함께 표시된다.** 이유는 우선순위를 둔다 —
데이터 타입 > 과제 유형 > 실행 위치. 서버가 없어도 수업은 계속된다.
...
**진행 표시는 모델 단위다.** 3개를 보내면 3개가 각각 보고한다.
묶음 전체 진행률은 클라이언트가 센다.

```jsonc
{ "batchId": "batch-2", "runId": "run-3", "algorithm": "svm", "stage": "TRAINING", "progress": 0.42 }
```
...
실패한 run은 지표도 모델도 없다. 비교표에는 실패 사유와 함께 남는다.
...
| — | 과제 유형의 한국어 표기 | `regression`을 "회귀"로 부를지 "예측"으로 부를지. 로케일 파일에서 정한다 |
