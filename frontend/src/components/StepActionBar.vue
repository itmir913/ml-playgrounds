 ...
 * 59). 붙박이가 지키려던 것은 오래 걸리는 동작의 진행 표시라, `below`가 있으면 바가
 ...
 *
 * **예측 화면만 예외다** — `sticky`를 받아 폭을 가리지 않고 통째로 붙는다(결정문 59의 예외).
 *
 ...
 * 0이고, 게이지 줄만 붙으면 그 줄의 높이고, `sticky`로 통째로 붙으면 바 전체다 — 안 그러면 도착 지점(`under-step-bar`)이 없는
