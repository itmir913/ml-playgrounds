 ...
 * 기다려 주지 않는다.
 *
 * **다만 이 보장에는 조건이 있다 — `action`이 실제로 비켜 줘야 한다.** 일이 한 작업(task)
 * 안에서 동기로 끝나면 꺼짐이 화면에 서기도 전에 풀리고, 그동안 쌓인 클릭이 전부 한 번씩
 * 더 돈다 (`screen.ts`의 `yieldToScreen`). 오래 걸리는 계산을 메인 스레드에서 돌린다면
 * 시작하자마자 한 번, 그리고 단위마다 한 번 양보하라.
 *
