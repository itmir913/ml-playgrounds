...
defineProps<{
  ...
  denominator?: string
  /**
   * 위로 여는가. **기본은 위다** — 이 컴포넌트의 트리거는 대개 표 머리글이고 그 아래가
   * 전부 값이라, 아래로 열면 설명을 읽는 동안 정작 설명이 걸린 숫자들이 가려진다.
   *
   * **고르는 축의 머리글에서는 아래다.** 그 아래에 있는 것은 선택지 목록인데
   * **팝오버가 그 목록을 이름까지 담아 설명하므로** 가려도 잃는 것이 없다. 반대로
   * 위로 열면 자리가 모자라 낮은 상자가 서고 그 안에서 굴리게 된다 — 그것이
   * `screen.ts`의 `prefersTop`이 적어 둔 대가이고, **`side`를 사실대로 고르면
   * 치르지 않아도 되는 대가다.**
   */
  side?: 'top' | 'bottom'
}>()
...
<template>
  <!--
    ...
    **여는 쪽은 부르는 자리가 정한다** (기본은 위다 — 위 `side` 참고). 자리가 모자라면
    `AppPopover`가 반대쪽을 보지만, **양쪽 다 모자라면 요청한 쪽으로 돌아온다**
    (`screen.ts`의 `prefersTop`) — 그래서 요청 자체가 맞아야 한다.
  -->
  <AppPopover size="wide" :side="side ?? 'top'">
