    ...
    <div class="flex min-h-0 flex-col p-6 md:p-8" :class="props.fill ? 'h-full' : ''">
      ...
      <!--
        ...
        **좌우로 한 칸 반씩 새어 나간다** (`-mx-1.5 px-1.5`, 2026-09-22에 사용자가
        실물에서 봤다). **입력칸의 링은 이제 칸 안쪽에 그려져 잘리지 않는다**(`styles/base.css`,
        architecture.md §8.4). 이 여백이 남는 것은 **버튼** 같은 다른 포커스 대상 때문이다 — 그것들의
        링은 여전히 요소 바깥에 그려져서 스크롤 상자 끝에 닿으면 잘린다. 음수 여백으로 상자를
