 ...
 *
 * **두 판을 재서 점마다 큰 쪽을 잡았다.** 작은 행에서 두 배까지 흔들렸는데(1,000행이
 * 7과 15), 그 자리는 값 자체가 작아 잡음이 크다. 같은 이유로 어느 한 판을 고르는 것이
 * 아니라 **큰 쪽**을 잡는다 — 이 표가 지키기로 한 방향이 그것이다.
 *
 * **분류: 상한이 아니다.**
 */
 ...
 *
 * **`k`는 지배적인 손잡이였다.** 비용이 `행 × k × 특성 × 반복`이라 `k`에 붙는데,
 * 결정문이 처음에 `k`를 *"시간을 크게 안 바꾸는 나머지"*로 묶어 두었다 — **그게
 * 틀렸다.** 손잡이가 2에서 20까지 열려 있고, 그 사이가 **8.4배**다.
 *
 * **선형으로 곱하지 않고 이 표를 보간한다.** 반복 횟수가 `k`마다 달라 per-k 비용이
 * 27~55ms로 흔들린다 — 곧은 선이 아니다.
 *
 * **분류: 상한이 아니다.**
 */
