 ...
 * 행이면 잘린 표가 답을 못 준다.
 *
 * **값이 `PREDICT_PAGE_SIZE`와 같지만 공유하지 않는다.** 오늘 같은 것은 우연이고
 * (둘 다 "한 화면에 얼마나 담아도 되는가"에 답한다), 갈릴 때 한쪽을 고치는 사람이
 * 다른 쪽을 대신 판단하게 하지 않는다 — (알고리즘 × 구현) 상한들과 같은 규칙이다.
 *
 * 전체 몇 행 중 몇 행인지는 화면이 함께 말한다.
 */
export const CLUSTER_MEMBER_PAGE_SIZE = 100
