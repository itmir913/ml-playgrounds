 ...
 *
 * **쟀다 (2026-08-11).** 개발 PC · Chromium · 캔버스 800×420 · 군집 5개, 화면과 같은
 * Chart.js 등록과 같은 옵션이다. **보는 것은 다시 그리기 시간이다** — 첫 그리기는 패널을
 * 열 때 한 번이지만 다시 그리기는 **학생이 축을 바꿀 때마다** 돈다.
...
/**
 * 군집 구성원 표에 보이는 줄 수 (open-decisions.md #28-6).
 *
 * **실측이 아니라 훑어보기 상수와 같은 근거다** — 위 `PREVIEW_ROW_COUNT`가 답하는
 * 질문("표를 눈으로 훑어 감을 잡는 데 몇 줄이 필요한가")과 같다. 값을 공유하지 않는
 * 이유는 (알고리즘 × 구현) 상한들과 같다: 근거가 같아 보여도 다음에 갈릴 값이고,
 * 공유하면 한쪽을 고치는 사람이 다른 쪽을 대신 판단하게 된다.
 *
