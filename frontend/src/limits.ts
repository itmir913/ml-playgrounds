 ...
 *
 * **아직 안 쟀다.** 10만 행을 그대로 Chart.js에 넘기면 저사양 학교 PC와 휴대폰에서
 * 무엇이 일어나는지 아무도 재지 않았고, 재기 전까지는 보수적으로 둔다. 넘으면
 * `randomState`로 표본을 뽑아 그리고 **화면이 그 사실을 말한다** — 조용히 일부만
 * 그리면 학생은 자기 데이터가 다 거기 있다고 믿는다.
 *
 * **이 상한은 그리는 점에만 걸린다** (#28-6). 군집 배정·군집 크기·구성원 표는 전체
 * 행에서 나온다 — 표본 안에서 고르면 "그 군집의 구성원"이 거짓말이 된다.
 */
export const CLUSTER_SCATTER_POINT_LIMIT = 2000
