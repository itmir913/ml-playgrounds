 ...
 *
 * **형식을 우리가 소유한다** (2026-09-18). 전에는 `z.iso.datetime({ offset: true })`였는데,
 * 그러면 **`.mlpx`가 받아들이는 시각의 집합을 zod의 내부 정규식이 정한다** — 그리고 그것이
 * 실제로 움직였다: 4.6.0이 초를 필수로 바꿔서 `2026-09-18T10:30Z`가 하루아침에 거부됐다.
 * 우리가 쓴 파일은 `toISOString()`이라 안 걸렸지만, **`.mlpx`는 이미 학생 손에 나간
 * 계약이고 그 계약이 의존성 갱신으로 좁아지면 안 된다.**
 *
 * **아래 정규식은 그날 zod가 내던 것 그대로다** — 손으로 옮겨 적지 않고 꺼내 심었다.
 * 초는 선택, 오프셋은 `Z`이거나 `±hh:mm`, 윤년의 2월 29일까지 본다.
 * 무엇을 받고 무엇을 거부하는지는 `tests/schema.spec.ts`가 표본으로 못 박는다.
 */
const TIMESTAMP =
  /^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z|([+-](?:[01]\d|2[0-3]):[0-5]\d)))$/

const timestamp = z.string().regex(TIMESTAMP).meta({ format: 'date-time' })
...
export const manifestSchema = z.looseObject({
   ...
   * 선택으로 내렸을 때와 같다. **반대 방향은 오른다** — 그 판정을 `tests/schema-structure.spec.ts`가
   * 구조 정본으로 대조한다 (mlpx-spec.md §9.2). 반대로 실험 스냅샷의 taskType은 여전히 필수다 -
...
export const imageDatasetRefSchema = z.looseObject({
   ...
   * 정본 한 변의 길이.
   *
   * **둘째 백본이 등록되는 날 카드를 잠그는 근거다** - 224로 구운 정본은 260을 요구하는
   * 백본에 못 간다. 없는 화소를 만들어 늘리지 않는다.
   *
   * **오늘은 아무도 이 값을 안 읽는다** (V11 R1 감사 B-2). 백본이 하나뿐이라 잠글 대상이
   * 없어서다 — 그러니 여기 적힌 값이 실제와 어긋나도 이 필드로는 안 걸린다. 실제로
   * 지키는 것은 임베딩 직전의 **사진 자신**이다: `ml/embed/runner.ts`가 정본이 백본이
   * 요구하는 크기인지 보고 아니면 멈춘다. **파일이 뭐라고 적혀 있든 화소가 이긴다.**
   */
...
export const settingsSchema = z.looseObject({
   ...
   *    (open-decisions.md "실행 방법은 (위치 × 엔진)이 아니라 하나의 목록이다") 그래서 run.engine을 기록한다.
   ...
   * (open-decisions.md "실행 방법은 (위치 × 엔진)이 아니라 하나의 목록이다"). 알고리즘 하나로만 키를 잡으면
