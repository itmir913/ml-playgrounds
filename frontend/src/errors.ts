...
export const CLIENT_WARNING_CODES = [
   ...
   *
   * **회귀에는 안 붙는다. 그쪽은 재고 나서 지표를 고쳤다** (2026-09-03). 상수 타깃으로
   * 회귀를 돌리면 참해가 `계수 0 · 절편 그 값`이라 R²가 1.0이어야 하는데, 우리 선형회귀
   * 라이브러리가 정규방정식을 써서 남긴 1e-14 먼지가 그것을 0.0으로 뒤집고 있었다.
   * sklearn처럼 타깃을 센터링해서 그 자리를 맞췄다(`ml/engines/mljs.ts`).
   */
