...
export const CLIENT_ERROR_CODES = [
   ...
   *
   * **`ENGINE_NOT_READY`와 나누는 이유는 사실이 다르기 때문이다.** 저쪽은 *"아직 안
   * 띄웠다"*는 상태이고 — 그리고 **정상 경로로는 안 나오는 그물이다**(바로 위) —
   * 여기는 *"띄우려다 실패했다"*는 사건이다. 가장 흔한 원인이 **학교망의 CDN 차단**이라
   * 다시 눌러도 같은 자리에서 죽는다. 그때 할 일은 순수 JS로 돌리는 것이다.
   *
   * **`BACKBONE_UNAVAILABLE`과도 다르다.** 사진 수업에는 대안이 없어서 그 문구가
   * *"다시 시도하세요"*인데, 여기는 **같은 알고리즘이 순수 JS로 돈다.**
   */
  'ENGINE_BOOT_FAILED',
...
export const SHARED_ERROR_CODES = [
  ...
  'FEATURE_HAS_MISSING',
  // 수치 열의 **시험 몫**에 숫자로 못 읽는 값이 있다. **빈 칸과 끝까지 나눈다** -
  // 학생이 할 일이 다르다(채우는 것과 고쳐 적는 것). 안 막으면 그 칸이 조용히 0이 되어
  // **정확도와 R²가 틀린 채로 포트폴리오에 적힌다** (2026-09-21 R36 A-1).
  // **지금 여기 닿는 것은 따로 올린 테스트 파일뿐이다** (2026-09-23, open-decisions.md 53) -
  // 같은 파일의 시험 몫은 열 종류 판정에 들어가서, 거기 글자가 있으면 열이 범주가 된다.
  'FEATURE_NOT_NUMBER',
  'TARGET_NOT_SELECTED',
...
export const CLIENT_WARNING_CODES = [
   ...
   *
   * **회귀는 아래 `TARGET_NO_VARIANCE`가 받는다.** 판정은 한 함수이고 코드만 갈린다.
   */
  'TARGET_TOO_FEW_CLASSES',
  /**
   * **회귀인데 타깃이 상수다 — 분산이 0이라 결정계수가 뜻을 잃는다**
   * (`ml/experiment.ts`의 `noVarianceWarning`, 2026-09-03 R25 §5).
   *
   * **위와 판정은 같고 학생이 할 일이 다르다.** 분류는 *"갈라 볼 것이 없다"*이고 이쪽은
   * **"오차로 판단하라"**다 — RMSE·MAE는 여전히 뜻이 있어서, 완벽히 맞힌 선형회귀(0.000)와
   * 크게 틀린 인공신경망(6.484)이 그 값으로는 갈린다. 결정계수만 둘 다 만점/영점으로 뭉갠다.
   *
   * **분모가 0일 때 R²는 sklearn 규칙으로 1.0 아니면 0.0이다**(`ml/metrics.ts`). 완벽히
   * 맞히면 **1.000**이 나오고, 교실에서 만점은 성공으로 읽힌다 — 분류의 100%와 같은 자리다.
   *
   * **문구가 "1.000이 나와도"라고 말하면 안 된다.** 같은 데이터에서 인공신경망은 0.000을
   * 받으므로 모델에 따라 그 문장이 거짓이 된다 (R25가 재서 잡았다).
   *
   * **세는 축이 분류와 다르다.** 회귀 타깃은 엔진이 `Number()`로 읽으므로 `42`·`42.0`이
   * 든 열은 문자열로는 두 종류인데 **수치로는 한 종류**다. 그 열에서 R²의 분모는 정확히
   * 0인데 문자열로 세면 경고가 안 붙는다.
   */
  'TARGET_NO_VARIANCE',
] as const
