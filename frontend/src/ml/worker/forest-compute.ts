 ...
 * 지어도 **라이브러리가 직렬로 지은 것과 같은 예측을 하는 숲**이 나온다.
 *
 * **"바이트 단위로 같다"가 아니다** (2026-09-04 R26 C-2). 아래 `plainTree`가 `root`에
 * JSON을 태우므로 노드의 `Matrix`가 배열이 되고, 되살린 쪽은 값이 같은 다른 물건이다.
 * 같은 것은 **모델 파일과 예측**이고 그 둘을 `forest-parallel.spec.ts`가 잰다.
 ...
 *
 * **직렬 경로는 여기를 안 지난다** (2026-09-04 R26 C-1 — 전에 이 자리의 주석이
 * 반대를 말했다). 풀이 `null`이면 `mljs.ts`가 라이브러리의 `train`을 그대로 부르므로
 * 직렬 숲은 **살아 있는 노드**이고 병렬 숲은 JSON 왕복본이다. 갈리는 것은 메모리
 * 위의 물건뿐이고 **모델 파일과 예측은 같다** — 그것이 "결과는 코어 수와
 * 무관하다"가 요구하는 전부이고, `forest-parallel.spec.ts`의 검사 둘이 그 둘을 잰다.
 */
