 ...
 * (`open-decisions.md` 64). 방향은 늘 내리막으로 잡지만 목적 함수가 수치적으로 더 못 내려가면
 * (평탄하거나 NaN·무한대) 역추적이 끝없이 돌 수 있어 묶는다 — 다 쓰면 선탐색 실패로 멈추고 지금
 * 기울기로 수렴 여부를 판정한다(아래 `minimize`의 선탐색). 1로 줄이면 `mljs.spec.ts`의 로지스틱
 * 대조가 운다 — 10으로는 안 운다(그 사이 문턱은 재지 않았다).
 */
