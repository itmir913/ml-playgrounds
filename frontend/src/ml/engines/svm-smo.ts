 ...
 *
 *    **실측하니 이 출구는 좀처럼 열리지 않는다 (2026-08-11, V3 감사 6단계와
 *    재검증).** 개발 PC(i5-1135G7)에서 겹침 10~50%의 800·1500·3000행, 그리고
 *    멀찍이 떨어진 두 덩어리(20~400행)까지 **열다섯 벌이 전부 정체 종료였다.**
 *    갈라치기의 α 변화량 문턱(`1e-3`)이 `tol`과 같은 규모라 마지막 위반이 그 아래로
 *    잘 안 깎이기 때문이다. **완전히 갈라지는 데이터에서는 드물게 열린다** —
 *    감사자의 독립 측정에서 여섯 벌 중 한 벌이 KKT 출구로 끝났다. 그래도
 *    **이 솔버의 실질적 종료 조건은 정체다.** 그 출구가 `converged: false`였다면
 *    교실의 거의 모든 SVM 학습에 경고가 붙었을 것이다 (open-decisions.md #26-1).
 *
