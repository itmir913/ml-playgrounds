 ...
 * 이 계층이 지키는 것 셋.
 *
 * 1. **파라미터는 훈련 데이터에서만 구한다.** 평균·최솟값·범주 목록을 전체 데이터에서 구하면
 *    테스트 데이터 정보가 학습으로 새어 지표가 부풀고, 학생은 자기 모델이 실제보다 좋다고 믿는다.
 ...
 *
 * **박스 플롯도 이것을 쓴다** (`data/stats.ts`, 2026-09-21). 사분위수를 구하는 규칙이
 ...
 * 범주가 되고 시험 몫이면 수치로 남아 거절됐다. `plan.ts`는 **그 실행이 쓰는 행**(훈련 ∪
 * 시험)을 넘긴다. **파일 전체가 아니다** — 쓰지 않는 행(타깃이 빈 행)의 글자가 모델을
 * 망치지 않게, 그리고 이미 낸 파일의 재실행이 안 흔들리게 그렇게 골랐다(결정문 53의
 * *"축을 바로잡는다"*, R38 B-1). 그래서 **파일 전체로 세는 데이터 화면의 요약과는 다를 수
 * 있고**, 전처리 화면은 이 종류를 따른다(R38 A-2). **종류는 통계가 아니라서 누출이
 * 아니다.** 부르는 쪽이 안 주면 예전처럼 훈련 몫이다.
 */
