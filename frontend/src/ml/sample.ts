 ...
 *    `Math.round`를 라벨마다 따로 쓰면 **라벨별 비율이 어긋난다.** 합은 안 어긋난다 —
 *    아래 2단계의 되받기가 도로 맞춘다(R7 감사가 잡은 과장이다). 어긋나는 것은
 *    "몇 줄인가"가 아니라 **"어느 라벨이 몇 줄인가"**이고, 층화의 뜻이 거기 있다.
 * 2. **바닥 올리기** — 1에서 `MIN_SPLIT_ROWS`에 못 미친 라벨을 끌어올리고, 그만큼을
 *    여유 있는 라벨에서 되받는다. 층화 분할이 라벨마다 훈련 데이터와 테스트 데이터에 하나씩은
 *    보내야 하기 때문이다.
 *
