...
const PANELS: readonly MetricPanel[] = [
  ...
  },
  {
     ...
     *
     * **사진에서도 선다** (2026-09-03에 `neural_network`가 사진을 열면서 함께 뒤집었다).
     * 곡선은 특성 이름을 안 쓰므로 임베딩 위에서도 뜻이 그대로다 — 계수 표가 표 데이터
     * 전용인 것과 갈리는 자리다. **사진 분류에는 혼동 행렬과 범주별 점수가 있지만,
     * 학습이 어떻게 진행됐는지를 말하는 것은 여전히 이 곡선뿐이다.**
     */
    id: 'loss-curve',
    dataTypes: { tabular: true, image: true },
    taskTypes: { classification: true, regression: true, clustering: false },
