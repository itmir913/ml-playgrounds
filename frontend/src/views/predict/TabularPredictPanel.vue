 ...
 * 바뀌면(새로 학습) 없어진 것만 떨구고 새것만 켠다 — 없어진 것을 계속 선택한 채로
 * 두면 아무것도 안 보이는 필터가 조용히 생긴다. 프로젝트를 바꾸면 처음부터다.
 */
...
/**
 * [무작위로 하나 가져오기]. 테스트에 쓴 행 중에서 무작위로 하나 준다.
 *
        ...
        <div v-if="inputMode === 'value'" class="flex min-h-96 flex-1 flex-col gap-5 md:flex-row">
          ...
          <!--
            **도착 지점이 바 아래에서 멈춘다.** 여백이 없으면 `scrollIntoView`가 목표를
            화면 맨 위에 붙여 놓는데, 그 자리는 붙박이 바가 덮고 있어(예측 화면이라 모든 폭에서 — 결정문 59의 예외)
            `모델들의 답`이 가려진다.
