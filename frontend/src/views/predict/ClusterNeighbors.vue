 ...
 *
 * **computed 안에서 만들면 안 된다** — 재료의 알맹이는 훈련 행렬이고 그건 파일에만
 ...
 *
 * 목록을 세우는 데 필요한 것은 **답과 모델 형식뿐**이고, 훈련 행렬은 아래에서 고른
...
/**
 * 고른 모델 하나의 설명. **차트도 훈련 행렬도 여기서만 산다** (#28-7).
 *
  ...
  <section v-if="neighborhood" class="flex min-w-0 flex-col gap-5">
    ...
    <!--
      **한 번에 하나만 그린다** (#28-7). 덩어리마다 그리면 군집 실험이 스물 쌓인 파일에서
      차트 스물이 서고 훈련 행렬 스물이 살아 있는다. **고를 것이 하나면 이 자리도 없다**
