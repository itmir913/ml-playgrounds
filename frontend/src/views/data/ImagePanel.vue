...
async function bake(): Promise<void> {
   ...
   * `project-switch-remount.spec.ts`는 이 판의 굽기 전 묶음과 다른 판의 굽는 중 전환을 잰다).
   * 이것은 그 키가 못 덮는 틈 — 파일이 바뀐 뒤 판이 내려가기 전 — 을 막는다. 옮겼으면 그
   * 묶음은 앞 프로젝트의 것이라 앉히지 않고 판에서도 접는다. 굽기 뒤의 확인은
   * `image-panel-drop.spec.ts`의 *"굽는 동안 다른 프로젝트로 옮기면"*이 문다. 굽기 전의
   * 확인은 사람 확인이다.
   */
