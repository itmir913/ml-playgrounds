 ...
 *
 * **[학습하기]가 끝나도 결과 화면으로 옮기지 않는다.** 설정을 바꿔가며 반복 학습하는 것이
 ...
 * (open-decisions.md "실행 방법은 (위치 × 엔진)이 아니라 하나의 목록이다"). 화면이 못 담는 조합을 애초에
 ...
 * 학생에게는 방금 학습이 돌았는지 아닌지가 남지 않는다. 그래서 [학습하기] 옆에 상시로 둔다.
 *
...
<template>
  <div v-if="settings" class="flex flex-col gap-5 p-4 sm:p-5">
    ...
    -->
    <!--
      **학습이 도는 동안에는 못 건드린다.** 실험은 [학습하기]를 누른 순간의 스냅샷으로 돌므로
    ...
    <!--
      ...
      **`<Transition>`은 저장소에 없다** (2026-09-02 R24 C-4에서 이 주석을 고쳤다).
      2026-08-05에 넣었다가 **작업 공간이 통째로 비고 `<main>`에 빈 주석 자리만 남아**
      되돌렸고(`App.vue`), 그때 배운 것이 *"화면마다 루트가 하나"*다. 다시 넣는 날을
      위해 그 규칙은 `tests/ui-rules.spec.ts`가 지킨다.
    -->
    <!--
