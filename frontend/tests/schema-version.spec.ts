 ...
 * **아직 한 번도 배포하지 않았다.**
 *
 * 첫 배포에서 이 값을 `true`로 바꾼다. 그 순간부터 어휘를 바꾸려면 버전을 올려야 하고,
 * 아래 지문 표의 `1:` 줄은 역사가 되어 고칠 수 없게 된다.
 *
 * **배포 워크플로가 이 값을 확인해야 한다** (`open-decisions.md` #10-1). 사람이 기억해서
 * 뒤집기를 기대하지 않는다 — 뒤집는 것을 잊으면 배포된 앱이 "아직 배포 전"이라고 믿는
 * 채로 어휘를 계속 바꾸게 되고, 그건 이 파일이 막으려는 바로 그 상태다.
 */
