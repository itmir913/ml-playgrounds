...
describe('결측 대체', () => {
  ...
  /**
   * **동점이면 가장 작은 값이 이긴다** (open-decisions.md 52, 2026-09-23).
   *
   * **전에는 반대를 못 박았다** — *"먼저 나온 값이 이긴다"*(R7 감사 B-7)였고, 그때 요지는
   * *"어느 쪽이든 정해져 있어야 같은 파일이 같은 답을 낸다"*였다. 그 요지는 그대로이고
   * **정해진 쪽만 sklearn으로 옮겼다.** `SimpleImputer(strategy='most_frequent')`를 sklearn
   * 1.9.1로 같은 입력에 돌리면 `부산`이 나온다.
   *
   * 입력 순서는 일부러 **`서울`이 먼저**다. 먼저 나온 값과 작은 값이 갈려야 이 판이 무엇을
   * 재는지가 선다 — 둘이 같은 입력이면 어느 규칙이어도 초록이다.
   */
  it('최빈값이 동점이면 가장 작은 값이 이긴다 - sklearn과 같다', () => {
    const tied: Dataset = {
      columns: ['지역'],
      rows: [['서울'], ['부산'], ['서울'], ['부산'], ['']],
    }
    const fitted = fitPreprocessor(tied, [0, 1, 2, 3, 4], ['지역'], preprocessing())
    expect(fitted.columns[0]?.fill).toBe('부산')
  })

  /**
   * **수치 열은 수로 견준다.** 문자열로 견주면 `10`이 `9`보다 작아진다. 전에는 수를
   * 문자열로 바꿔 최빈값을 구했는데, 동점 규칙이 *"먼저 나온 값"*이라 그 차이가 안 드러났다.
   * **작은 값을 고르게 된 지금부터는 드러나므로** 함께 못 박는다(sklearn은 `9.0`이다).
   */
  it('수치 열의 동점은 수로 작은 값이다', () => {
    const tied: Dataset = {
      columns: ['점수'],
      rows: [['10'], ['9'], ['10'], ['9'], ['']],
    }
    const fitted = fitPreprocessor(
      ...
      [0, 1, 2, 3, 4],
      ['점수'],
      preprocessing({ missing: 'mostFrequent' }),
    )
