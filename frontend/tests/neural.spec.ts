...
import { loadModel, loadModelProba, NEURAL_FORMAT, parseNeural } from '../src/ml/models'
...
describe('역전파가 손실의 기울기와 같다', () => {
  ...
  it('학습이 손실을 내린다', () => {
    ...
    const fitted = fitNeural(features, encoded, 2, { hiddenLayers: 1, neuronsPerLayer: 8 }, 42)
  ...
  it('곡선의 마지막 값이 그 가중치의 실제 손실이다', () => {
    const { features, encoded } = twoBlobs()
    const fitted = fitNeural(features, encoded, 2, { hiddenLayers: 1, neuronsPerLayer: 6 }, 3)
  ...
  it('해석적 기울기가 유한차분과 맞는다', () => {
    ...
    const start = fitNeural(features, encoded, 2, { hiddenLayers: 2, neuronsPerLayer: 5 }, 5)
    const weights = start.weights.map((matrix) => matrix.map((row) => [...row]))
    const intercepts = start.intercepts.map((bias) => [...bias])

    const analytic = neuralGradientForTest(weights, intercepts, features, encoded)
...
describe('씨앗이 결과를 정한다', () => {
  it('같은 씨앗이면 같은 곡선이다', () => {
    ...
    const a = fitNeural(features, encoded, 2, options, 42)
    const b = fitNeural(features, encoded, 2, options, 42)
  ...
  it('다른 씨앗이면 다른 곡선이다 - 초기화가 도착점을 정한다', () => {
    ...
    const options = { hiddenLayers: 2, neuronsPerLayer: 5 }
    const a = fitNeural(features, encoded, 2, options, 42)
    const b = fitNeural(features, encoded, 2, options, 7)
...
describe('손잡이 둘이 층 모양을 정한다', () => {
  it('은닉층 수 + 1개의 가중치 덩어리가 나온다', () => {
    const { features, encoded } = twoBlobs()
    const fitted = fitNeural(features, encoded, 2, { hiddenLayers: 3, neuronsPerLayer: 7 }, 42)
  ...
  it('다중 클래스의 출력은 클래스 수만큼이다', () => {
    ...
    const fitted = fitNeural(features, encoded, 3, { hiddenLayers: 1, neuronsPerLayer: 4 }, 42)
