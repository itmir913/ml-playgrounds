 ...
 * 2. **잎의 분포를 합이 1이 되게 고쳐 읽는다.** sklearn은 `tree_.value`를 이미 정규화해서
 *    주므로(**1.8과 1.9 둘 다 그렇다** — 2026-09-19 R31이 1.8.0을 깔아 확인했다) 나눗셈이
 *    없는 것과 같고, 그래서 정규화를 지워도 조용했다.
 *    **그래도 나누는 것은 파일이 sklearn에서만 오지 않기 때문이다** — 손으로 고친 파일과
 *    다른 엔진이 표본 수를 담으면 정규화 없이는 **표본이 많은 나무가 더 크게 말한다.**
 *
