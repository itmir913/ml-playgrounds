 ...
 *
 * **주석은 부르는 쪽이 미리 걷는다** (`withoutComments`). 여기서 다시 세면 상태 기계가
 * 두 벌이 된다.
 *
 * **문자열은 안 걷힌다. 알고 두는 구멍이다** (2026-09-01 R18 감사 C-2). 한때 이 자리가
 * *"문자열과 주석은"*이라고 적었는데 **절반이 거짓이었다** — `withoutComments`가 스스로
 * *"문자열 리터럴 안은 건드리지 않는다"*고 적는다. 그래서 본문에 `'}'` 같은 홀 중괄호가
 * 든 문자열이 있으면 **거기서 잘못 자른다.** 지금 쓰는 자리(`src/limits-switch.ts`)에는
 * 그런 문자열이 없고, **구멍의 크기는 `tests/source-tools.spec.ts`가 못 박는다.**
 *
 * @param open 여는 `{`의 자리
 */
