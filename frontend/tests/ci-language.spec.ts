 ...
 * | `throw new Error('...')` | 스택과 함께 찍힌다 |
 *
 * **소스가 아니라 `tests/`를 본다.** `src`는 `i18n-usage.spec.ts` "src의 .ts 따옴표 리터럴에
 * 한글이 없다"가 막는다 — **throw 자리만이 아니라 따옴표 리터럴 전부다.** 한국어를 변수에
 * 먼저 담아 던지면 throw 바로 뒤에는 따옴표가 없기 때문이다(`i18n.ts`의 빠진 키 메시지는
 * 검사 모드에서 던져져 CI 로그에 찍힌다). 여기가 비어 있던 탓에 119개가 한국어로 쌓여
 * 있었다 (2026-09-01에 옮겼다).
 */
