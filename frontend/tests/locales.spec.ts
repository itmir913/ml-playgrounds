...
describe('프런트엔드 전용 코드', () => {
  ...
  it('지어낸 순우리말이 한국어 문구에 없다', () => {
    // CLAUDE.md 3 규칙 5 · copy.md 2. **순우리말이 금지가 아니라 짓는 것이 금지다** -
    // `불러오기`·`지우기`는 한글과 윈도가 쓰는 굳은 말이라 여기 없다.
    //
    // **이 목록은 실제로 거절당한 낱말만 갖는다.** 후보를 미리 채우면 쓸 수 있는 말까지
    // 막고, 그러면 다음 사람이 목록을 안 믿는다. 낱말이 거절될 때마다 한 줄이 는다.
    const COINED = [
      '꾸러미', // 2026-08-28. zip은 `압축 파일`이다 - 에러 문구가 이미 그렇게 부른다
      '견주', // copy.md 2의 첫 줄. `비교`다
    ]
    for (const [key, value] of korean) {
      for (const word of COINED) {
