...
describe('프런트엔드 전용 코드', () => {
  ...
  it('동작의 이름에 순우리말을 쓰지 않는다', () => {
    ...
    //
    // **활용형까지 적는다.** 처음에는 `채우기`만 적었는데 `채웁니다`가 그대로 살아남아
    // 선택지는 `최빈값으로 대체하기`인데 그 아래 설명은 `가장 많은 값으로 채웁니다`였다.
    // **검사는 통과했고 화면을 열어야 보였다.** 어간이 다른 낱말과 겹치지 않으면 어간으로,
    // 겹치면(`담`은 `부담`·`담당`이 있다) 형태를 하나씩 적는다.
    //
    // **못 보는 것: 목록에 없는 새 순우리말.** 허용 목록이 닫혀 있다는 것은 사람만 안다.
    const RETIRED = [
      '꾸러미', // 2026-08-28. zip은 `압축 파일`이다 - 에러 문구가 이미 그렇게 부른다
      '견주', // copy.md 2의 첫 줄. `비교`다
      '떼', // 2026-08-29 훑기의 계기. 테스트 데이터를 떼는 것은 `해제`다
      '지우', // `삭제`. 2026-08-29 아침까지 이 검사가 봐주던 말이다
      '지워',
      '지웁',
      '고르', // `선택`. 체크리스트는 이미 `선택하기`였는데 문장만 갈려 있었다
      '골라',
      '고릅',
      '뽑', // 표본은 `추출`, 난수 씨앗은 `새로 만들기`
      '채우', // 결측치는 `대체`
      '채워',
      '채웁',
      '다듬', // 데이터 정제는 `정제`, 경계는 `조정`
      '내려받', // 브라우저가 부르는 말은 `다운로드`다
      '섭니다', // `한 줄로 섭니다`는 비유였다. 예측 결과는 열로 선다
      '답을 냅', // `예측합니다`
      '답을 낸',
      // 아래는 어간이 다른 낱말과 겹쳐서 형태를 적는다.
      '담기', // 장바구니 은유였다. `추가`
      '담은',
      '담아',
      '담았',
      '담지',
      '담을',
      '빼기', // `제거`. 목록에서 빠지는 것과 없어지는 것을 가른다
      '빼야',
      '뺍니다',
      '맞추기', // 층화는 `범주 비율 유지`. `범위를 맞춥니다`는 비유가 아니라 그냥 동사다
      '맞춰',
      '나누기', // `분할`. terms.md가 `훈련/테스트 분할`로 못 박았다
      '나눌',
      '나눕',
      '나누는',
    ]
