 ...
 * 로케일 파일 사이의 계약.
 *
 * **jsdom 선언을 뗐다** (2026-08-19, R7 감사 B-14). 근거가 *"양식 출처 등록부를 통해
 * `i18n.ts`에 닿는다"*였는데 **이 파일은 그 등록부를 더 이상 import하지 않는다.**
 * 전이 도달로도 DOM 가드에 안 닿고, 떼고 돌려도 마흔여섯 개가 전부 통과한다.
 *
 *
...
describe('프런트엔드 전용 코드', () => {
  ...
  // CLAUDE.md 3 규칙 5 · copy.md 2. **동작의 이름은 한자어이고, 순우리말은 copy.md 2의
  // 닫힌 목록에 있는 것만 쓴다** - `불러오기`·`내보내기`·`만들기`·`되돌리기`·`미리보기`·
  // `끌어다 놓기`가 그 목록이라 여기 없다.
  //
  // **아래는 전부 실제로 걷어낸 낱말이다.** 2026-08-29 이전에는 "거절당한 말만" 담았고
  // 무엇이 거절될지는 사람이 판정했다 - 그 판정이 갈려서 `떼기`가 들어왔다. 이제는
  // 훑으면서 물러난 말이 그대로 줄이 된다.
  //
  // **활용형까지 적는다. 어간 접두 매칭은 한국어에서 새어 나간다.**
  // `지우다`의 `지웠습니다`는 `지우`로 시작하지 않는다 - 어미가 어간의 마지막 음절과
  // 한 글자로 합쳐지기 때문이다(지우 + 었 -> 지웠). 처음에 `채우기`만 적었더니
  // `채웁니다`가 그대로 살아남아, 선택지는 `최빈값으로 대체하기`인데 그 아래 설명은
  // `가장 많은 값으로 채웁니다`였다. **2239개가 다 초록인 채로 그랬고 화면을 열어야
  // 보였다.** 같은 구멍으로 `지웠습니다`·`매긴`·`매길` 셋이 더 나왔다.
  //
  // **한자어와 겹치는 어간은 형태를 하나씩 적는다** - `담`은 `부담`·`담당`을 잡는다.
  // 넣기 전에 지금 로케일에서 0건인지 확인한다. 오탐이 한 번 나면 다음 사람이 목록
  // 자체를 안 믿는다.
  //
  // **못 보는 것: 목록에 없는 새 순우리말.** 허용 목록이 닫혀 있다는 것은 사람만 안다.
  //
  // **셋은 남기기로 했다** (2026-09-01, 코드 소유자) — `모드로 바꾸기`(배색) ·
  // `멈추기`(학습) · `머무르기`(나가기 확인). **한자어가 오히려 교실에서 낯설다**:
  // `전환`·`중지`·`잔류`는 중고등학생이 화면에서 만날 말이 아니고, 그러면 규칙의 목적
  // (학생이 기능이 아니라 낱말에서 막히지 않게)에 어긋난다. **`copy.md` §2의 닫힌
  // 목록에 올려 두었으므로 다음 사람이 같은 판단을 다시 하지 않는다.**
  const RETIRED = [
    '꾸러미', // 2026-08-28. zip은 `압축 파일`이다 - 에러 문구가 이미 그렇게 부른다
    '견주', // copy.md 2의 첫 줄. `비교`다
    // **`견주`가 `견줄`을 못 잡는다** (2026-08-31). 위 머리말이 경고한 어미 합쳐짐인데
    // 정작 이 줄이 그 짝을 안 들고 있었고, `계수의 크기를 서로 견줄 수 없습니다`가
    // 2,516개 초록을 통과해 화면에 떴다. **줄을 세울 때 그 낱말의 -ㄹ/-ㅂ 꼴부터 적어라.**
    '견줄',
    '견줍',
    '견줘',
    '견줬',
    '견준',
    '견줌',
    // 떼다 -> `해제`. 2026-08-29 훑기의 계기다
    '떼',
    '뗀',
    '뗐',
    // 지우다 -> `삭제`. 2026-08-29 아침까지 이 검사가 봐주던 말이다
    '지우',
    '지워',
    '지웁',
    '지웠',
    '지울', // 2026-08-30 R12 감사. 어간 접두로는 -ㄹ까요 꼴이 안 잡힌다
    // 고르다 -> `선택`. 체크리스트는 이미 `선택하기`였는데 문장만 갈려 있었다
    '고르',
    '골라',
    '골랐',
    '고릅',
    // 뽑다 -> 표본은 `추출`, 난수 씨앗은 `새로 만들기`. 활용형이 어간을 안 깬다
    '뽑',
    // 채우다 -> 결측치는 `대체`
    '채우',
    '채워',
    '채웁',
    '채웠',
    // 매기다 -> `평가`. 점수를 매긴다는 말이 세 자리에 남아 있었다
    '매기',
    '매긴',
    '매길',
    '매겼',
    // 다듬다 -> 데이터는 `정제`, 경계는 `조정`
    '다듬',
    // 내려받다 -> 브라우저가 부르는 말은 `다운로드`다
    '내려받',
    // 담다(장바구니 은유) -> `추가`. 어간이 `부담`·`담당`과 겹쳐 형태를 적는다
    '담기',
    '담은',
    '담아',
    '담았',
    ...
    '담긴',
    '담겼',
    // 빼다 -> `제거`. 어간 한 글자는 너무 넓다
    '빼기',
    '빼야',
    '뺍',
    '뺐',
    // 층화는 `범주 비율 유지`. **`맞추다` 자체는 살아 있다** - `값 범위를 비슷하게
    // 맞춥니다`는 비유가 아니라 그냥 동사다. 물러난 것은 그 자리의 이름뿐이다
    '맞추기',
    '비율을 맞춰',
    // 나누다 -> `분할`. terms.md가 `훈련/테스트 분할`로 못 박았다
    '나누기',
    '나눌',
    '나눕',
    '나눴',
    '나누는',
    '나눠',
    '나누었', // 2026-08-31 사각 감사. `나눴`의 안 줄인 꼴이 내장 양식에 살아 있었다
    // 고치다 -> 포트폴리오는 `편집`이고 그 끝맺음은 `수정완료`다 (2026-09-01, 코드
    // 소유자). **`수정`이 아니라 `편집`인 것은 그 화면이 문서이기 때문이다** — 문항을
    // 통째로 쓰고 지우고 옮기는 자리라 한 낱말을 손보는 `수정`보다 넓다
    '고치',
    '고쳐',
    '고칩',
    '마치기',
    // 옮기다 -> `이동`. 위·아래로 문항을 나르는 자리다
    '옮기',
    '옮겨',
    // 비우다 -> `삭제`. 그 자리의 영어가 `Clear`이고 지우는 것이 맞다
    '비우기',
    '비웁',
    // 들어가다 -> `열기`. **닫힌 목록에 이미 있는 말인데 다른 낱말을 쓰고 있었다**
    '들어가기',
    // 쓰다 -> 단추의 이름은 `작성`이다(`이어서 작성`, 2026-09-26, 코드 소유자). 문장 안의
    // `쓰세요`·`쓴 글`은 일반 동사라 남는다 - 물러난 것은 동작의 이름 꼴 하나다
    '쓰기',
    // 바꾸다 -> 무엇을 바꾸느냐로 갈린다. `이름 변경` · `데이터/파일 교체` ·
    // `하이퍼파라미터 조정`. **배색 스위치의 `모드로 바꾸기`는 남긴다** — 아래 참조
    '이름 바꾸기',
    '데이터 바꾸기',
    '파일 바꾸기',
    ') 바꾸기',
    // 재다 -> `측정`. **화면은 이미 그렇게 부르고 있었다** (`preprocess.testDataRemove
    // Description`의 `다른 것을 측정한 값`) - 2026-09-01에 상한 문구만 갈려 있었고
    // 사용자가 잡았다. **어간 한 글자는 `현재`·`존재`·`소재`를 잡으므로 형태를 적는다.**
    //
    // **지표 설명문도 맞췄다** (2026-09-01, 코드 소유자). 처음에는 `거리를 재서 평균 낸
    // 값`이 바로 아래 줄의 `평균 낸 값` 면제와 한 문장에 있어 `재서`를 뺐는데, **면제된
    // 것은 `내다`이지 `재다`가 아니다** - 같은 개념을 한쪽은 `측정`, 한쪽은 `재서`라고
    // 부르면 그것이 곧 갈라짐이다. `metricHelp.rmse`·`silhouette` 둘을 함께 고쳤다.
    '재어',
    '재 보',
    '재서',
    '재고',
    '잰 ',
    // 비유로 쓴 것들. **`평균 낸 값`·`찾아낸 비율`은 안 잡는다** - 지표 설명문의
    // 관용이고, copy.md 2가 손대지 않기로 한 자리다
    '섭니다',
    '답을 냅',
    '답을 낸',
  ]
  ...
  /**
   * **학생이 읽는 한국어는 로케일 JSON만이 아니다.** 내장 양식(`public/portfolio/`)의
   * 안내문도 그렇고, 그쪽은 이 검사가 안 훑고 있었다 — 옆의 `terms.spec.ts`는 정확히
...
describe('화면이 부르는 키가 로케일에 있다', () => {
  ...
  const PAIRED_PREFIXES: readonly string[] = [
    ...
    'entryHash.', //   〃
    'reproduction.', // 재실행 대조의 판정마다 문구가 있다 (점검 화면의 대조 판)
    'inspect.blocked.', // 대조를 막는 이유마다 문구가 있다 (`reproduceBlockers`)
...
describe('두 언어가 나란히 말한다', () => {
  ...
  const ALLOWED: readonly (readonly string[])[] = [
     ...
     * `-하기`(`삭제하기`·`해제하기`·`추가하기`)다. 옛 문구도 전부 `-기`였다
     * (`지우고 바꾸기`·`다시 뽑기`).
     *
     * **영어에는 그 자리를 표시할 어미가 없다.** 명령형 하나가 버튼과 확인을 다
     * 덮으므로 여기서 짝이 모인다. 한국어 쪽을 영어에 맞춰 뭉개면 **어느 것이
     * 되돌릴 수 없는 확인인지가 화면에서 사라진다.**
     */
    [
    ...
    // 한국어는 수를 표시하지 않아 한 낱말로 모인다.
    ['data.image.deletePhotos', 'portfolio.removePhoto'],
