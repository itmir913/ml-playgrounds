 ...
 * 그때는 이름이 있다고 보는 쪽으로 틀린다. 스크립트 상수에 든 클래스, 순서가 뒤집힌
 * `class="sm:inline hidden"`은 못 본다. `aria-labelledby`로 준 이름은 이름으로 안 쳐서 없다고
 * 보는 쪽으로 틀린다. 실제로 숨는지는 CSS라 여기서 못 본다(사람 확인).
 */
