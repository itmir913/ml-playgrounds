...
describe('군집', () => {
  ...
  /**
   * **채워진 군집이 셋이어야 `b(i)`가 뜻을 갖는다.**
   *
   * 이 파일의 다른 군집 픽스처는 전부 실제로 채워지는 군집이 **둘**이라 "다른 군집"이
   * 하나뿐이고, 그러면 `Math.min`과 `Math.max`가 **항등**이다. 실제로 `b(i)`를
   * "가장 가까운"에서 "가장 먼" 군집으로 뒤집어도 저장소 전체가 초록이었다
   * (R13-2 감사 A-2).
   *
   * **기대값은 밖에서 왔다.** 정의(`s = (b - a) / max(a, b)`)에서 점마다 손으로 세운
   * 것이고 우리 함수를 안 지난다 — 셋을 한 줄에 세워 거리가 눈으로 세어지게 했다.
   * 뒤집으면 `b`가 가장 먼 군집이 되어 0.978이 나온다.
   */
  it('군집이 셋이면 가장 가까운 군집으로 잰다', () => {
    const spread = [
      [0, 0],
      [1, 0],
      [10, 0],
    ...
    const byHand = (9.5 / 10.5 + 8.5 / 9.5 + 8.5 / 9.5 + 9.5 / 10.5 + 38.5 / 39.5 + 39.5 / 40.5) / 6
