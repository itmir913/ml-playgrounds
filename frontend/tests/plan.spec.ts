...
describe('옵션이 층화를 막으면 무시한다', () => {
  ...
  it('회귀면 뽑기도 층화하지 않는다', () => {
    const dataset: Dataset = {
      columns: ['x', 'y'],
      rows: [...Array(30).keys()].map((index) => [String(index), String(index % 3)]),
    }
    const run = (stratify: boolean) =>
      planRun({
        dataset,
        testDataset: null,
        settings: settingsFor(
          { features: ['x'], target: 'y' },
          { nSamples: 12, split: { method: 'holdout', testSize: 0.3, stratify, randomState: 7 } },
        ),
        taskType: 'regression',
      })
    const on = run(true)
