 ...
 * ─────────────────────────────────────────────────────────────────────────
 *
 * **배포 전에는 "올리지 마라"였다.** 버전이 하는 일은 옛 파일과 새 파일을 가르는
 * 것인데 밖에 나간 파일이 하나도 없었으니 숫자가 뜻을 못 가졌다. 형식이 바뀌어 기존
 * 테스트 프로젝트가 안 맞게 되면 마이그레이션을 설계하지 말고 그 `.mlpx`를 지웠다.
 *
 * **2026-08-15에 배포하면서 규칙이 뒤집혔다** (`tests/schema-version.spec.ts`의
 * `RELEASED`가 `true`다). 이제 이 파일이 뜻하는 것은 **"움직이지 마라"가 아니라
 * "지시 없이 움직이지 마라"**다 — 어휘나 좌표계가 바뀌면 숫자는 **반드시** 올라가야
 * 하고, 그때 이 파일을 같은 커밋에서 함께 고친다. 위 상자의 판정 기준은 그대로다:
 * **지시가 있었는가.**
 *
 * `FORMAT_VERSION`이 2로 오른 것이 그 첫 사례다 (2026-08-19, mlpx-spec.md §9.1).
 */
